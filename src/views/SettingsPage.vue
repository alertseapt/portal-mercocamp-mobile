        ...
        <div class="card-header">
          <div class="header-left">
            ...
            <p class="section-subtitle">
              Configure notificações e destinatários de e-mail
            </p>
          </div>
        </div>
        ...
        <div class="email-settings-content">
            ...
            <div class="setting-control">
              <div class="email-display">
                ...
                <span class="email-value">{{
                  emailSettings.primaryEmail || 'Nenhum e-mail configurado'
                }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="setting-group">
            <div class="setting-header">
              ...
              <p class="setting-description">
                Adicione e-mails que devem receber cópia das notificações
              </p>
            </div>
          ...
          <div class="setting-group">
            <div class="setting-header">
              ...
              <p class="setting-description">
                Configure quais mudanças de status devem gerar e-mails com a
                notificação
              </p>
            </div>
...
const VueApiClient =
  ...
  class {
    ...
    async testEmailSettings() {
      return this.request('/user-settings/test-email', { method: 'POST' })
    }
...
export default {
  ...
  computed: {
    allStatusNotificationsEnabled() {
      return Object.values(this.emailSettings.statusNotifications).every(
        enabled => enabled
      )
    },
  ...
  methods: {
      ...
      if (userData) {
        try {
          ...
          if (Number(user.level_access) === 4) {
            console.log(
              '⚙️ SettingsPage: Usuário nível 4 (Manutenção) - pulando carregamento de configurações'
            )
    ...
    async testEmailSettings() {
      if (!this.emailSettings.primaryEmail) {
        this.$emit(
          'notification',
          'E-mail principal é obrigatório para testar',
          'warning'
        )
        return
      }
      ...
      try {
        ...
        if (
          this.emailSettings.ccEmails &&
          this.emailSettings.ccEmails.length > 0
        ) {
    ...
    addCcEmail() {
      if (
        this.newCcEmail.trim() &&
        !this.emailSettings.ccEmails.includes(this.newCcEmail.trim())
      ) {
    ...
    async saveEmailSettings() {
      ...
      try {
        this.$emit(
          'notification',
          'Salvando configurações de e-mail...',
          'info'
        )
        ...
        this.$emit(
          'notification',
          response.message || 'Configurações de e-mail salvas com sucesso!',
          'success'
        )
      } catch (error) {
    ...
    validatePasswordForm() {
      ...
      } else if (this.passwordForm.newPassword.length < 4) {
        this.passwordErrors.newPassword =
