  ...
  methods: {
    ...
    async handleGenerateTXT() {
      if (!this.canGenerateTXT) {
        this.$emit(
          'notification',
          'Selecione NFes do mesmo cliente para gerar o arquivo TXT. Funcionalidade disponível apenas para usuários com nível de acesso diferente de 1.',
          'error'
        )
        return
      }

      console.log('CorpemTxtGenerator - Iniciando geração e download do TXT...')
      this.setLoading(
        true,
        'Gerando TXT...',
      ...
      try {
        const result = await generateCorpemTXT(
          this.user,
          this.selectedScheduleObjects
        )
        ...
        this.$emit(
          'notification',
          `Erro ao gerar arquivo TXT: ${error.message}`,
          'error'
        )
      }
      ...
      console.log(
        'CorpemTxtGenerator - Forçando reset do bulkActionLoading no pai'
      )
    ...
    setLoading(isLoading, message = '', detail = '') {
      console.log('CorpemTxtGenerator - setLoading called:', {
        isLoading,
        message,
        detail,
      })
